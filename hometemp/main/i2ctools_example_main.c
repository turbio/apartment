...
void sendCommand(uint16_t command, uint16_t arguments, bool arg) {
    ...
    ESP_ERROR_CHECK(i2c_master_write_byte(cmd, (ESP_SLAVE_ADDR << 1) | I2C_MASTER_WRITE, ACK_CHECK_EN));
...
uint8_t ReadFromSCD30(uint16_t command, uint16_t *val, uint8_t cnt) {
    ...
    ESP_ERROR_CHECK(i2c_master_write_byte(cmd, (ESP_SLAVE_ADDR << 1) | I2C_MASTER_READ, ACK_CHECK_EN));
...
void init_http(void) {
    esp_http_client_config_t config = {
        .url = "http://turb.io:9091/metrics/job/esp32",
...
void app_main(void) {
    ...
    sendCommand(COMMAND_AUTOMATIC_SELF_CALIBRATION, 1, true); // enable self calibration
